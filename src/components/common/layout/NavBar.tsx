...
const NavBar = () => {
	...
	// New function to handle user account icon click
	const handleAccountClick = () => {
		...
		switch (user.userRole) {
			...
			case "ADMIN":
				...
				break;
			default:
				navigate("/account"); // Fallback to generic account page
		}
			...
			<nav className="container mx-auto px-4 sm:px-6 lg:px-8" aria-label="Top">
				<div className="flex items-center justify-between h-16">
					...
					<div className="hidden md:flex items-center space-x-8">
						<Link
							to="/products"
							className="text-gray-600 hover:text-construction-blue">
							Products
						</Link>
						<Link
							to="/about"
					...
					<div className="hidden md:flex items-center space-x-4">
						{isAuthenticated ? (
							<>
								...
								{/* Updated to use handleAccountClick */}
								<Button
									variant="ghost"
									size="icon"
							...
							<div className="border-t border-gray-200 pt-4 mt-2">
								{isAuthenticated ? (
									<>
										{/* Updated to use handleAccountClick */}
										<div
											className="flex items-center text-gray-600 hover:text-gray-900 px-3 py-2 cursor-pointer"
											onClick={handleAccountClick}>
